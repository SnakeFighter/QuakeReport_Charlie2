...
public final class QueryUtils {
    ...
    //TODO get the real json response into this method.
    //TODO remove references to static string JSON.
    private QueryUtils(String jsonResponse) {
    }
    ...
    public static ArrayList<Earthquake> extractEarthquakes(String jsonResponse) {
        ...
        Log.i("QueryUtils","extractEQs");

        // Create an empty ArrayList that we can start adding earthquakes to
        ArrayList<Earthquake> downloadedEarthquakes = new ArrayList<>();
        ...
        try {
            ...
            //Loop through the features to get the earthquake data we need.
            for (int i = 0; i < featuresArray.length(); i++) {
                JSONObject thisFeature = featuresArray.getJSONObject(i);
                JSONObject properties = thisFeature.getJSONObject("properties");
                double mag = properties.getDouble("mag");
                String place = properties.getString("place");
                long time = properties.getLong("time");
                String url = properties.getString("url");

                Earthquake thisEarthquake = new Earthquake(mag, place, time, url);

                downloadedEarthquakes.add(thisEarthquake);
            }
